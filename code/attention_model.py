...
class AttentionModel(BaseModel):
    ...
    def call(self):
        ...
        """
        ########################## NEW ###############################
        encoder_output, att_weights = SelfAttention(size=128,
                                                     num_hops=10,
        ...
        # out = Flatten(name='Flatten')(out) # 0.74 como máximo.
        # out = Dropout(rate=self.rate, name='dropout')(out)
        ...
        self.model = Model(inputs=[sequence_input], outputs=[prediction], name='self-attention-model')
        self.model.compile(loss=BinaryCrossentropy(),
                           optimizer=self.optimizer,
                           metrics=self.METRICS)

        self.model.summary()
        tf.keras.utils.plot_model(self.model, show_shapes=True, dpi=48, to_file='self-attention-model.png')
