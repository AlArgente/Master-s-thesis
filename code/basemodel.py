...
class BaseModel(Layer):
    ...
    def __init__(self, max_len, path_train, path_test, path_dev, batch_size=32, embedding_size='300', emb_type='fasttext',
    ...
    def load_data(self):
        ...
        self.train['label'][self.train['label']==-1] = 0
        self.test = pd.read_csv(self.path_test, sep='\t')
        self.test['label'][self.test['label'] == -1] = 0
        self.dev = pd.read_csv(self.path_dev, sep='\t')
        self.dev['label'][self.dev['label'] == -1] = 0
